...
3. `class Smartphone` 
является результатом классического наследника, class Product который добавляет дополнительные характеристики в своей категории товаров.
4. `class LawnGrass` 
является классом наследника, class Product который добавляет дополнительные характеристики в своей категории товаров.
5. `class BaseProduct` 
Родительский класс для class Product Определяет общую функциональность для классов наследников.
## Структура проекта
